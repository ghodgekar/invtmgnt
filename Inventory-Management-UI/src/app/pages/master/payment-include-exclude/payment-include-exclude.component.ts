...
export class PaymentIncludeExcludeComponent {
  ...
  public view: any = 'payment-include-exclude';
  created_by: any;
  created_at: any;
  updated_by: any;
  updated_at: any;
  ...
  getPaymentData(){
    this.common.list('payment-mode').subscribe((res:any) => {
      this.paymentModeData = res.lists
    })
  }

  getTransaction(event:any, isEdit?:any){
    if(isEdit == 1){
    ...
    if(this.transactionType == 1){
      this.common.list('category').subscribe((res:any) => {
        this.transactionData = res.lists
      })
    }
  ...
  onSubmit(): void {
    ...
    if (this.paymentInclExclForm.invalid) {
      ...
      if(this.submitBtn == 'SAVE'){
        ...
        this.common.save( this.paymentInclExclForm.value, this.view).subscribe((res:any) => {
        ...
        this.common.update(this.paymentInclExclForm.value, this.view).subscribe((res:any) => {
          $('#evaluator_table').DataTable().ajax.reload();
          this.isEdit = false;
          this.submitBtn = 'SAVE';
          this.onReset();
  ...
  editPaymentInclExclList(id: any){
    ...
    this.common.edit(id, this.view).subscribe((res:any) => {
      ...
      this.getTransaction(this.transactionType, 1);
      this.paymentInclExclForm.patchValue({
